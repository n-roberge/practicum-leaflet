...
const ORIGIN = [0, 0];
const log = (obj) => console.log("[georaster-layer-for-leaflet] ", obj);
// figure out if simple CRS
// even if not created with same instance of LeafletJS
const isSimpleCRS = (crs) => crs === L.CRS.Simple ||
    (!crs.code &&
        crs.infinite &&
...
const GeoRasterLayer = L.GridLayer.extend({
    ...
    getRasters: function (options) {
        const { innerTileTopLeftPoint, heightOfSampleInScreenPixels, widthOfSampleInScreenPixels, zoom, numberOfSamplesAcross, numberOfSamplesDown, ymax, xmin } = options;
        if (this.debugLevel >= 1)
        ...
        */
        ...
        if (!Object.values(getValuesOptions).every(isFinite)) {
            ...
            return Promise.all(this.georasters.map((georaster) => georaster.getValues({ ...getValuesOptions, resampleMethod: this.resampleMethod || "bilinear" }))).then(valuesByGeoRaster => valuesByGeoRaster.reduce((result, values) => {
    ...
    drawTile: function ({ tile, coords, context, done }) {
        try {
            ...
            if (debugLevel >= 3)
                console.log("[georaster-layer-for-leaflet] extent of inner tile before snapping " +
            ...
            if (debugLevel >= 3)
                console.log("[georaster-layer-for-leaflet] extent of inner tile after snapping " +
                    extentOfInnerTileInMapCRS.reproj(inSimpleCRS ? "simple" : 4326).bbox.toString());
            ...
            if (debugLevel >= 3)
                log({ innerTileHeight, innerTileWidth });
            if (debugLevel >= 4) {
    ...
    _isValidTile: function (coords) {
        ...
        if (!crs.infinite) {
            ...
            if ((!crs.wrapLng && (coords.x < globalBounds.min.x || coords.x > globalBounds.max.x)) ||
     ...
     */
    updateColors(pixelValuesToColorFn, { debugLevel = -1 } = { debugLevel: -1 }) {
